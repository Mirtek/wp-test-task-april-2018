...
function create_our_first_metabox_callback(){
	...
	$value = get_post_meta(get_the_ID(), '__movie_subtitle', true );
	?>
	<div class="metabox-comtainer">
		<div class="single-metabox">
		<label for="">Movie subtitle </label>
		<input type="text" id="abt_author" name="movie_subtitle" value="<?php
			if(!empty($value)){
				echo $value;
...
function save_our_metabox_value(){
	...
	if(!isset($_POST['movie_subtitle'])){
		return;
		
	}
	
	update_post_meta(get_the_ID(), '__movie_subtitle', sanitize_text_field($_POST['movie_subtitle'])); 
	
	
}
add_action( 'save_post', 'save_our_metabox_value' );

function create_our_second_metabox(){
	add_meta_box(
			'our_second_meta',
			'Category',
			'create_our_second_metabox_callback',
			'movie'			
	);
}

add_action('add_meta_boxes','create_our_second_metabox');
// meta box callback
function create_our_second_metabox_callback(){
	wp_nonce_field( basename(__FILE__),  'nonce_name_formeta2' );
	$value = get_post_meta(get_the_ID(), '__movie_category', true );
	?>
	<div class="metabox-comtainer">
		<div class="single-metabox">
		<label for="">Movie category </label>
		<input type="text" id="movie_category" name="movie_category" value="<?php
			if(!empty($value)){
				echo $value;
			}
...
function save_our_second_metabox_value(){
	if(!wp_verify_nonce( $_POST['nonce_name_formeta2'], basename(__FILE__)  )){
		return;
	}
	...
	update_post_meta(get_the_ID(), '__movie_category', sanitize_text_field($_POST['movie_category'])); 
	
	
}
add_action( 'save_post', 'save_our_second_metabox_value' );
